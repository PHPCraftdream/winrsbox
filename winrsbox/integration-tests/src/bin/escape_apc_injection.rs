// Escape payload: NtQueueApcThread on foreign process thread.
// Expected: terminated by inject_guard (QueueApc to non-owned process).

fn main() {
    eprintln!("[escape_apc] starting");
    ...
    unsafe {
        ...
        // Direct NtQueueApcThread to foreign thread
        let ntdll_w: Vec<u16> = "ntdll.dll\0".encode_utf16().collect();
        let hntdll = winapi::um::libloaderapi::GetModuleHandleW(ntdll_w.as_ptr());
        let queue_apc = winapi::um::libloaderapi::GetProcAddress(
            hntdll, b"NtQueueApcThread\0".as_ptr() as *const i8,
        );
        ...
        let nt_queue: Fn = std::mem::transmute(queue_apc);

        // Use ExitThread as APC routine (benign target, doesn't matter — hook should kill us first)
        ...
        let status = nt_queue(
            thread_h as *mut _, exit_thread as *mut _,
            std::ptr::null_mut(), std::ptr::null_mut(), std::ptr::null_mut(),
        );
        eprintln!("[escape_apc] NtQueueApcThread returned 0x{status:x} (should be killed)");
    }
}
