...
/// Workspace root (`…/winrsbox`), the parent of this crate's manifest dir.
fn workspace_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .expect("integration-tests manifest dir has a parent")
        .to_path_buf()
}
...
fn source_crates_for(file_name: &str) -> &'static [&'static str] {
    match file_name {
        "winrsbox.exe" => &["launcher", "policy", "ipc"],
        "hook.dll" => &["hook", "policy", "ipc"],
        // Everything else here is an integration-test payload binary.
        _ => &["integration-tests", "policy", "ipc"],
    }
}
