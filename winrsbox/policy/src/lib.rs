...
pub struct RegistryPolicy {
    ...
    cache: Cache<u128, Arc<RegDecision>>,
    reg_snapshot: arc_swap::ArcSwap<RegSnapshot>,
    overlay: std::sync::Mutex<reg_overlay::RegOverlay>,
}

struct RegSnapshot {
    rules: Vec<SnapshotRule>,
    default_rule: Option<db::RuleRow>,
    mocks: rustc_hash::FxHashMap<String, Vec<u8>>,
}
...
impl RegistryPolicy {
    ...
    pub fn decide_with_context(
        ...
        let lower = ensure_lower(key_path);
        let vname = value_name.map(|v| ensure_lower(v));
