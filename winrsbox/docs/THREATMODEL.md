...
| IPC-spawn-proxy escape (sandboxed process commands an external service to spawn for it) | Medium | Partially closed by denylists: `com_guard` (Shell.Application, Schedule.Service, WMI, WSH, FileSystemObject, ...), `alpc_guard` (schedule/wmi/wbem/spool/dcomlaunch/appinfo/...), `shell_guard` (`ms-*:` / `runas:` / shell URIs). NOT fully closed for arbitrary user-session named pipes — e.g. a pre-existing wezterm-mux-server, ssh-agent, or VS Code remote-host process that runs OUTSIDE the sandbox can be commanded via its named pipe to spawn a child on the requester's behalf; that child is parented by the broker (not by us) and is NOT in our Job. **Per-application workaround**: known multi-instance shells get a "no-attach" flag in `shell install` (wezterm: `--always-new-process` forces an in-process mux owned by THIS sandboxed instance). **Structural closure**: separate Window Station + named-pipe namespace per headless sandbox (roadmap, same as the UI-isolation note below). |
