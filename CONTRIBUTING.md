...
| Crate | Role |
|-------|------|
| `winrsbox/launcher` | the `winrsbox.exe` launcher + CLI + policy management |
| `winrsbox/hook` | the injected `hook.dll` — ntdll hooks, CoW redirect, guards |
| `winrsbox/policy` | policy engine: glob rules, decisions, path/registry classification |
| `winrsbox/ipc` | hook ↔ launcher IPC protocol (bincode over a named pipe) |
| `winrsbox/integration-tests` | escape proof-of-concept binaries (not `cargo test`s) |
